  ...
  Widget build(BuildContext context) {
    return Consumer<BiometricsViewModel>(
      builder: (context, vm, child) {
        if (vm.isLoading) {
          return Scaffold(body: Shimmer.fromColors(baseColor: Colors.grey[300]!, highlightColor: Colors.grey[100]!, child: const Center(child: Text('Loading...'))));
        }
        if (vm.error != null) {
          return Scaffold(body: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(vm.error!), ElevatedButton(onPressed: vm.loadData, child: const Text('Retry'))])));
        }
                ...
                child: Column(
                  children: [
                    Row(
                      ...
                      children: [7, 30, 90].map((d) => ElevatedButton(onPressed: () {
                        vm.setRange(d);
                      ...
                      }, child: Text('${d}d'))).toList(),
                    ),
                    SwitchListTile(title: const Text('Large Dataset (10k+)'), value: vm.largeDataset, onChanged: vm.toggleLargeDataset),
